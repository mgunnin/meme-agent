...
import { ProcessedTokenData, TokenSecurityData } from "../types/token.js"
...
export class TrustScoreManager {
  ...
  async generateTrustScore(
    ...
    const recommenderMetrics = await this.trustScoreDb.getRecommenderMetrics(
      recommenderId
    )
  ...
  calculateTrustScore(tokenPerformance: any, recommenderMetrics: any): number {
  ...
  calculateOverallRiskScore(tokenPerformance: any, recommenderMetrics: any) {
    const riskScore = this.calculateRiskScore(tokenPerformance)
    const consistencyScore = this.calculateConsistencyScore(
      tokenPerformance,
      recommenderMetrics
  ...
  async getRecommendations(
    ...
    const recommendations =
      await this.trustScoreDb.getRecommendationsByDateRange(startDate, endDate)
    ...
    const processedResults = await Promise.all(
      Object.keys(groupedRecommendations).map(async (tokenAddress) => {
        ...
        const recommendationResults = await Promise.all(
          tokenRecommendations.map(async (recommendation) => {
    ...
    return processedResults.sort(
      (a: TokenRecommendationSummary, b: TokenRecommendationSummary) =>
