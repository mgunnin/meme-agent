...
export const marketAnalysisGroup = {
  ...
  description:
    "A group of agents specialized in market analysis and trading decisions",
...
export class MarketAnalysisCoordinator {
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        if (attempts < maxAttempts) {
          await this.delay(
            marketAnalysisGroup.workflow.fallbackBehavior.backoffMs
          )
        }
      } catch (error) {
        elizaLogger.error(
          `Error in stage ${stage.name}, attempt ${attempts + 1}:`,
          error
        )
        attempts++
      }
  ...
  private calculateConfidence(results: any[]): number {
    return (
      results.reduce((acc, result) => acc + (result.confidence || 0), 0) /
      results.length
    )
  }
