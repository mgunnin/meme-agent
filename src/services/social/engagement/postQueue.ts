...
export class PostQueue extends EventEmitter {
  ...
  public async addToQueue(
    post: Omit<QueuedPost, "id" | "status" | "createdAt" | "retryCount">
  ): Promise<string> {
  ...
  public getQueueStatus(): {
    ...
    return {
      ...
      processing: posts.filter((p) => p.status === PostStatus.PROCESSING)
        .length,
  ...
  public getPostsByStatus(status: PostStatus): QueuedPost[] {
    return Array.from(this.queue.values())
      ...
      .sort(
        (a, b) => b.priority - a.priority || a.scheduledTime - b.scheduledTime
      )
  }
    ...
    try {
      try {
        ...
        if (!content && post.platform === "twitter") {
          const prompt =
            `Generate a fun, tweet-ready message about ${post.metadata.category}` +
            (post.metadata.tags.length
              ? ` including tags: ${post.metadata.tags.join(", ")}`
              : "") +
            (post.metadata.campaign
              ? ` for campaign: ${post.metadata.campaign}`
              : "")
