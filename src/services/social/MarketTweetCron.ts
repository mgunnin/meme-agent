...
import { JupiterSwap, MarketData } from "../../types/market.js"
...
export class MarketTweetCron {
  ...
  public async start(options: MarketTweetOptions): Promise<void> {
    try {
      ...
      elizaLogger.info(
        `Market tweet cron started with ${
          options.interval || this.DEFAULT_INTERVAL
        }ms interval`
      )
    } catch (error) {
  ...
  private async verifyServices(): Promise<void> {
    ...
    try {
      await this.jupiterService.getTokenPrice(
  ...
  private async postMarketUpdate(tokenAddress: string): Promise<void> {
    for (let attempt = 1; attempt <= this.MAX_RETRIES; attempt++) {
      try {
        ...
        const holderInfo = await this.heliusService.getHoldersClassification(
          tokenAddress
        )
        if (!holderInfo) {
        ...
        const enhancedMarketData: MarketData = {
          ...
          volume24h: tokenPrice.extraInfo?.quotedPrice?.buyPrice
            ? Number(tokenPrice.extraInfo.quotedPrice.buyPrice) *
          ...
          onChainActivity: {
            ...
            uniqueTraders: new Set(
              recentSwaps.map((swap: JupiterSwap) => swap.data.author_id)
            ).size,
          },
        ...
        elizaLogger.success(
          "Market update tweet posted successfully:",
          tweet.content
        )
        ...
        elizaLogger.error(
          `Market update attempt ${attempt}/${this.MAX_RETRIES} failed:`,
          error
        )
