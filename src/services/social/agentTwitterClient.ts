...
export class AgentTwitterClientService {
    ...
    for (let i = 0; i < retries; i++) {
      try {
        ...
        if (i > 0) {
          ...
          console.log(
            `Waiting ${Math.round(delay / 1000)} seconds before attempt ${
              i + 1
            }...`
          )
        ...
        try {
          ...
          for (let verifyAttempt = 0; verifyAttempt < 3; verifyAttempt++) {
            try {
              ...
              console.warn(
                `Login verification attempt ${verifyAttempt + 1} failed:`,
                error
              )
            }
            // Increase delay between verification attempts
            await new Promise((resolve) =>
              setTimeout(resolve, 15000 * (verifyAttempt + 1))
            )
          }
          ...
          if (error?.message?.includes('code":399')) {
            console.log(
              "ACID challenge detected, implementing extended delay..."
            )
  ...
  private ensureInitialized(): void {
    if (!this.isInitialized || !this.scraper) {
      throw new Error(
        "AgentTwitterClientService not initialized. Call initialize() first."
      )
    }
  }
