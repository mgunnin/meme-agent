    ...
    try {
      ...
      console.log("Generating tweet with context:", {
        marketCondition: marketData
          ? this.determineMarketCondition(marketData)
          : "unknown",
        style,
      ...
      if (
        marketData &&
        (isNaN(marketData.price) || isNaN(marketData.volume24h))
      ) {
      ...
      const maxContentLength =
        maxLength - importantElements.length - emojiStr.length - 1 // -1 for space
...
- Trading Volume: $${(marketData.volume24h / 1000000).toFixed(
        2
      )} million in last 24h
...
- Trading Pattern: ${
        marketData.onChainData?.recentSwaps !== undefined
          ? marketData.onChainData.recentSwaps > 10
            ? "Active market participation"
            : marketData.onChainData.recentSwaps > 5
            ? "Steady trading flow"
...
- Recent Engagement: ${
        communityMetrics.activeUsers24h
      } members active in past 24 hours
- Market Confidence: ${communityMetrics.sentimentScore.toFixed(
        2
      )} confidence rating
...
- Keep it concise and professional (max ${
      constraints?.maxLength || this.DEFAULT_MAX_LENGTH
    } chars)
...
- Present on-chain activity data when relevant\n`
  ...
  async generateThreadFromMarketUpdate(
    marketData: MarketData
  ): Promise<string[]> {
