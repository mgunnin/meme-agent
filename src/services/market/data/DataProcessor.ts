...
export class MarketDataProcessor {
  ...
  constructor(heliusApiKey: string, jupiterPriceUrl: string) {
  ...
  public async getMarketData(tokenAddress: string): Promise<MarketData> {
    ...
    try {
      ...
      const holders = (await this.heliusService.getHoldersClassification(
        tokenAddress
      )) as {
        totalHolders: number
  ...
  public async calculateAverageVolatility(
    tokenAddress: string,
    window: number
  ): Promise<number> {
    ...
    for (let i = 1; i < historicalPrices.length; i++) {
      const priceChange = Math.abs(
        historicalPrices[i].price - historicalPrices[i - 1].price
      )
  ...
  private calculatePriceChange(priceData: any): number {
    ...
    const lastPrice = Number(
      priceData.extraInfo.lastSwappedPrice.lastJupiterSellPrice
    )
  ...
  private async calculateVolatility(tokenAddress: string): Promise<number> {
    ...
    try {
      ...
      const { buyPriceImpactRatio, sellPriceImpactRatio } =
        prices.extraInfo.depth

      // Calculate volatility using price impact ratios
      const avgImpact =
        (Object.values(buyPriceImpactRatio.depth).reduce((a, b) => a + b, 0) +
          Object.values(sellPriceImpactRatio.depth).reduce(
            (a, b) => a + b,
            0
          )) /
      ...
      const cached = await this.redis.get(cacheKey)
      if (cached) {
        const { volatility } = await this.decompressData<{
          volatility: number
        }>(cached)
        return volatility
      }
