...
export class PriceMonitor extends EventEmitter {
  ...
  constructor(dataProcessor: MarketDataProcessor, aiService: AIService) {
    super()
  ...
  public async startMonitoring(
    ...
    try {
      ...
      elizaLogger.info(
        `Started monitoring ${tokenAddress} with ${interval}ms interval`
      )
    } catch (error) {
  ...
  private async updateTokenPrice(tokenAddress: string): Promise<void> {
    ...
    while (retryCount < this.MAX_RETRY_ATTEMPTS) {
      try {
        ...
        retryCount++
        elizaLogger.warn(
          `Retry ${retryCount} failed for ${tokenAddress}:`,
          error
        )

        if (retryCount === this.MAX_RETRY_ATTEMPTS) {
          elizaLogger.error(
            `Failed to update price after ${retryCount} attempts:`,
            error
          )
        ...
        case "change":
          if (previousPrice) {
            const change = Math.abs(
              (pricePoint.price - previousPrice) / previousPrice
            )
  ...
  private async analyzeMovement(
    ...
    if (Math.abs(priceChange) >= this.VOLATILITY_THRESHOLD) {
      ...
      const analysis = await this.generatePriceAnalysis(
        tokenAddress,
        priceChange,
        metrics
      )
  ...
  public calculateTokenMetrics(tokenAddress: string): TokenMetrics | null {
    ...
    const past24h = history.filter(
      (p) => p.timestamp > Date.now() - 24 * 60 * 60 * 1000
    )

    const dayAgo =
      history.find((p) => p.timestamp <= Date.now() - 24 * 60 * 60 * 1000) ||
      history[0]
