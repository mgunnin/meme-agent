...
export class TradingService {
  ...
  async getMarketData(tokenAddress: string): Promise<MarketData> {
    try {
      const marketData = await this.marketDataProcessor.getMarketData(
        tokenAddress
      )
      return {
        tokenAddress,
        holders: marketData.holders || { total: 0, top: [] },
        onChainActivity: marketData.onChainActivity || {
          transactions: 0,
          swaps: 0,
          uniqueTraders: 0,
        },
        ...
        volatility:
          typeof marketData.volatility === "object"
  ...
  private async calculateVolatility(tokenAddress: string): Promise<{
  ...
  private async getRecentActivity(
    publicKey: PublicKey
  ): Promise<{ transactions: number; swaps: number; uniqueTraders: number }> {
