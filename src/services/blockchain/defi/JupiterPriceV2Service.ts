...
export class JupiterPriceV2Service {
  ...
  private readonly SOLANA_PUBLIC_KEY: PublicKey = new PublicKey(
    process.env.SOLANA_PUBLIC_KEY ||
      "C7DjuqwXZ2kZ2D9RMDXv5HjiR7PVkLFJgnX7PKraPDaM"
  )
  ...
  public async getMarketMetrics(symbol: string): Promise<MarketMetrics> {
    try {
      ...
      await this.cache.set(cacheKey, JSON.stringify(marketMetrics), {
  ...
  public async getTokenPrice(symbol: string): Promise<TokenPrice | null> {
    try {
      ...
      await this.cache.set(cacheKey, JSON.stringify(tokenPrice), {
        expires: JupiterPriceV2Service.CACHE_TTL,
      })
  ...
  public async getMarketData(symbol: string): Promise<{
    ...
    try {
      ...
      await this.cache.set(cacheKey, JSON.stringify(marketData), {
        expires: JupiterPriceV2Service.CACHE_TTL,
      })
  ...
  public async getTokenInfo(symbol: string): Promise<TokenInfo | null> {
    try {
      ...
      await this.cache.set(cacheKey, JSON.stringify(tokenInfo), {
        expires: JupiterPriceV2Service.CACHE_TTL,
      })
  ...
  public async getHighestVolumeTokens(
    limit: number = 10
  ): Promise<TokenMovement[]> {
    try {
