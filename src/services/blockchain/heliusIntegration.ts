...
export class HeliusService {
  ...
  private async makeRequest<T>(
    ...
    const cacheKey = `helius:${method}:${JSON.stringify(
      params
      ...
      try {
        const response = await fetch(this.baseUrl, {
          ...
          body: JSON.stringify({
            ...
            params: [
              ...params,
              { commitment: options.commitment || "finalized", ...options },
            ],
          }),
        })
  ...
  async getAssetsByOwner(ownerAddress: string): Promise<any> {
    return this.makeRequest("getAssetsByOwner", [
      ownerAddress,
      { page: 1, limit: 100 },
    ])
  }
  ...
  async isHealthy(): Promise<boolean> {
    ...
    return (
      health.status === "ok" ||
      (health.status === "behind" && (health.distance || 0) < 100)
    )
  }
}
