...
export const CONFIG = {
  // Blockchain Settings
  SOLANA: {
    NETWORK: (process.env.SOLANA_NETWORK || "mainnet-beta") as NetworkType,
  ...
  AUTOMATION: {
    CONTENT_GENERATION_INTERVAL: parseInt(
      getRequiredEnvVar("CONTENT_GENERATION_INTERVAL")
    ),
    MARKET_MONITORING_INTERVAL: parseInt(
      getRequiredEnvVar("MARKET_MONITORING_INTERVAL")
    ),
    COMMUNITY_ENGAGEMENT_INTERVAL: parseInt(
      getRequiredEnvVar("COMMUNITY_ENGAGEMENT_INTERVAL")
    ),
  },
...
function validateConfig() {
  ...
  if (
    !Object.values(NetworkType).includes(CONFIG.SOLANA.NETWORK as NetworkType)
  ) {
  ...
  if (
    isNaN(CONFIG.SOLANA.TOKEN_SETTINGS.DECIMALS) ||
    CONFIG.SOLANA.TOKEN_SETTINGS.DECIMALS < 0
  ) {
  ...
  if (
    isNaN(CONFIG.SOLANA.TRADING.BASE_AMOUNT) ||
    CONFIG.SOLANA.TRADING.BASE_AMOUNT <= 0
  ) {
    throw new Error("Invalid trading base amount.")
  }
  if (
    isNaN(CONFIG.SOLANA.TRADING.MIN_CONFIDENCE) ||
    CONFIG.SOLANA.TRADING.MIN_CONFIDENCE < 0 ||
    CONFIG.SOLANA.TRADING.MIN_CONFIDENCE > 1
  ) {
    throw new Error("Invalid trading minimum confidence.")
  }
  if (
    isNaN(CONFIG.SOLANA.TRADING.SLIPPAGE) ||
    CONFIG.SOLANA.TRADING.SLIPPAGE < 0 ||
    CONFIG.SOLANA.TRADING.SLIPPAGE > 1
  ) {
  ...
  if (
    isNaN(CONFIG.AI.GROQ.DEFAULT_TEMPERATURE) ||
    CONFIG.AI.GROQ.DEFAULT_TEMPERATURE < 0 ||
    CONFIG.AI.GROQ.DEFAULT_TEMPERATURE > 1
  ) {
  ...
  if (
    isNaN(CONFIG.AUTOMATION.CONTENT_GENERATION_INTERVAL) ||
    CONFIG.AUTOMATION.CONTENT_GENERATION_INTERVAL <= 0
  ) {
    throw new Error("Invalid content generation interval.")
  }
  if (
    isNaN(CONFIG.AUTOMATION.MARKET_MONITORING_INTERVAL) ||
    CONFIG.AUTOMATION.MARKET_MONITORING_INTERVAL <= 0
  ) {
    throw new Error("Invalid market monitoring interval.")
  }
  if (
    isNaN(CONFIG.AUTOMATION.COMMUNITY_ENGAGEMENT_INTERVAL) ||
    CONFIG.AUTOMATION.COMMUNITY_ENGAGEMENT_INTERVAL <= 0
  ) {
  ...
  if (
    isNaN(CONFIG.MARKET.UPDATE_INTERVAL) ||
    CONFIG.MARKET.UPDATE_INTERVAL <= 0
  ) {
    throw new Error("Invalid market update interval.")
  }
  if (
    isNaN(CONFIG.MARKET.PRICE_CHANGE_THRESHOLD) ||
    CONFIG.MARKET.PRICE_CHANGE_THRESHOLD < 0 ||
    CONFIG.MARKET.PRICE_CHANGE_THRESHOLD > 1
  ) {
    throw new Error("Invalid market price change threshold.")
  }
  if (
    isNaN(CONFIG.MARKET.VOLUME_CHANGE_THRESHOLD) ||
    CONFIG.MARKET.VOLUME_CHANGE_THRESHOLD < 0 ||
    CONFIG.MARKET.VOLUME_CHANGE_THRESHOLD > 1
  ) {
  ...
  if (CONFIG.DEV.ERROR_REPORTING.ENABLED) {
    if (!CONFIG.DEV.ERROR_REPORTING.WEBHOOK_URL) {
      console.warn(
        "Warning: Error reporting is enabled but no webhook URL is provided. Disabling error reporting."
      )
